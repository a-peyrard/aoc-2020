...
With 10 cards, the top card is worth the value on the card multiplied by 10. In this example, the winning player's
score is:
...
def play_game(deck1: Deck, deck2: Deck) -> int:
    while deck1 and deck2:
        card1 = deck1.popleft()
        card2 = deck2.popleft()
        _play_higher_value_round(card1, card2, deck1, deck2)

    return _calculate_score(deck1 if deck1 else deck2)


def _play_higher_value_round(card1: int,
                             card2: int,
                             deck1: Deck,
                             deck2: Deck) -> None:
    if card1 > card2:
...
def play_recurse_game(deck1: Deck, deck2: Deck) -> int:
    winner, *decks = _play_recurse_game_rec(deck1, deck2, 1)
    return _calculate_score(decks[winner])


def _play_recurse_game_rec(deck1: Deck, deck2: Deck, depth: int) -> Tuple[int, Deck, Deck]:
    round_idx = 1
    known_states = set()
    while deck1 and deck2:
        DEBUG and print(f"""-- Round {round_idx} (Game {depth}) --
Player 1's deck: {deck1}
Player 2's deck: {deck2}
""")
        state = _generate_game_state(deck1, deck2)
        ...
        if card1 <= len(deck1) and card2 <= len(deck2):
            max_length_of_deck = card1 + card2
            winner, _, _ = _play_recurse_game_rec(
                deque(islice(deck1, 0, card1), maxlen=max_length_of_deck),
                deque(islice(deck2, 0, card2), maxlen=max_length_of_deck),
                depth + 1
            )
            if winner == 0:
                deck1.append(card1)
