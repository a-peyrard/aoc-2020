...
the secret encryption key that the card and door use to communicate; this would let you send the unlock command directly
 to the door!
...
def _parse(lines: List[str]) -> Tuple[int, int]:
    return int(lines[0].strip()), int(lines[1].strip())


def _do_loop(subject: int,
             loop_size: int,
             initial: int = 1) -> int:
    value = initial
...
def _find_loop_size(subject: int,
                    expected_key: int,
                    initial: int = 1,
    ...
    raise ValueError(f"Unable to find the loop size after {max_iterations} iterations :(")


def solve_part1(card_pub_key: int, door_pub_key: int) -> int:
    card_loop_size = _find_loop_size(
        subject=7,
        expected_key=card_pub_key,
        max_iterations=10000000
    )
    print(f"loop size found for card pub key is: {card_loop_size}")
    return _do_loop(
...
if __name__ == "__main__":
    with open(os.path.join(os.path.dirname(__file__), "input")) as file:
        _card_pub_key, _door_pub_key = _parse(list(file.readlines()))

        start = time.time()
        solution_part1 = solve_part1(_card_pub_key, _door_pub_key)
        end = time.time()
        print(f"solution (part1): {solution_part1} in {(end - start) * 1000}ms")
        assert solution_part1 == 15467093
