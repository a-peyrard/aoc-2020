...
def count_valid_passports(raw_entries: Iterator[str], required_fields: Set[str]) -> int:
    return sum(
        map(
            lambda p: is_valid_passport(p, required_fields),
            map(
                generate_passport,
...
def is_valid_passport(entries: Dict[str, str], required_fields: Set[str]) -> bool:
    return all((f in entries for f in required_fields))
...
if __name__ == "__main__":
    with open(os.path.join(os.path.dirname(__file__), "input")) as file:
        solution_part1 = count_valid_passports(file.readlines(), REQUIRED_PASSPORT_FIELDS)
